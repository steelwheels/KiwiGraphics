...
public func UTLayerMapper() -> Bool
{
	...
	let res1 = addObject(layer: layer, x:  0.0, y:  0.0, radius: 1.0, expectedIndex: KGLayerIndex(depth: 3, index:  0))
	let res2 = addObject(layer: layer, x: 40.0, y: 40.0, radius: 1.0, expectedIndex: KGLayerIndex(depth: 0, index:  0))
	let res3 = addObject(layer: layer, x: 22.0, y: 22.0, radius: 1.0, expectedIndex: KGLayerIndex(depth: 3, index: 12))
	let res4 = addObject(layer: layer, x: 62.0, y: 62.0, radius: 1.0, expectedIndex: KGLayerIndex(depth: 3, index: 60))
	let res5 = addObject(layer: layer, x: 50.0, y: 50.0, radius: 1.0, expectedIndex: KGLayerIndex(depth: 2, index: 12))
	let res6 = addObject(layer: layer, x: 50.0, y: 60.0, radius: 1.0, expectedIndex: KGLayerIndex(depth: 1, index:  3))
